...
> El health raíz reportaba `status: DOWN` porque el **indicador de SMTP falla** (no hay servidor de correo configurado; `MAIL_HOST` vacío). Se resolvió con `management.health.mail.enabled: false` (+ `show-details: always` para diagnóstico). Verificado en la nube: `/actuator/health` → `UP` con db (PostgreSQL), redis (7.4.11), ping, diskSpace, ssl, livenessState y readinessState todos `UP`. Ver 3.5 y Apéndice A.
...
| `/actuator/health` del identity → `DOWN` sin detalle; DB/Redis OK (login funciona)                                                                                                  | El `MailHealthIndicator` (activado por `spring-boot-starter-mail`) falla porque `MAIL_HOST` está vacío en la nube → baja el health compuesto                                                                                                | `management.health.mail.enabled: false` (y `show-details: always` para diagnosticar). Verificado: `/actuator/health` → `UP` (db/redis/ping/ssl/livenessState/readinessState). Ver 3.5.                                                       |
| Tras `docker push` + `terraform apply`, la revisión del servicio **NO se actualiza** (sigue en un digest viejo)                                                                     | Terraform compara el atributo `image` como string; con `:latest` el string no cambia entre builds (tag mutable), así que no hay `~` que disparar el redespliegue          | Usar un **digest inmutable** en `main.tf` (`image@sha256:...`) en vez de `:latest`; así cada push con nuevo digest sí genera cambio. Verificar la revisión activa con `gcloud run revisions list`.                                               |
| `/actuator/health` de `book-service`/`social-service`/`notification-service` → `503 DOWN` aunque la app funciona                                                                  | El **`MongoHealthIndicator`** falla: el ping al MongoDB (URI de la BD, no `admin`) no responde, bajando el health compuesto — inocuo, los endpoints de negocio funcionan | Intencionalmente **dejado como está** (decisión del equipo). Los 503 solo afectan al `/actuator/health` para diagnosis; no usa los probes de Cloud Run. Ver 3.X y Apéndice A. |
