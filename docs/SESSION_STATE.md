...
- **Fase 13 (despliegue cloud con Terraform) — alcance A desplegado y verificado** en GCP: Cloud SQL + Artifact Registry + Cloud Run identity (con sidecar Redis) + gateway + IAM público. `register` (directo a identity, 201) y `login` (vía gateway, 200) funcionando con JWT + rate-limit reales. Commiteado en Git (documentación `GUIDE-INFRA.md`, `SESSION_STATE.md`, `GUIDE.md` + infra estructura Terraform).

### Blocked

- Ninguno actualmente. Pendientes de decisión (no bloquean): user/book en la nube (requieren Mongo + RabbitMQ con cuentas gratuitas externas), Google OAuth2 real, y frontend (Bloque 4).

## Next Move

La **Fase 13 (deploy cloud Terraform) — alcance A (identity + gateway + Redis sidecar + Cloud SQL)** está desplegada y verificada. Continuación posible:

| Opción | Descripción |
|--------|-------------|
| **Seguir Fase 13 — user/book** | Desplegar user-service y book-service: requieren **MongoDB + RabbitMQ**. Opciones gratuitas: **MongoDB Atlas M0** + **CloudAMQP** (plan lemur); luego nuevos `google_cloud_run_v2_service` para ambos + actualizar `USER_SERVICE_URI`/`BOOK_SERVICE_URI` en el gateway (quitar los placeholders) + Mongo/AMQP URIs en `terraform.tfvars`. |
...
| Nota | El `package.json` raíz (basura heredada, sin uso) ya está **ignorado** en el `.gitignore` raíz. El frontend real usa `frontend/package.json`. `gcloud` se ejecuta en el terminal del usuario (Git Bash/Windows PowerShell), no siempre disponible en la shell del agente. |
...
- `infrastructure/terraform/environments/dev/main.tf` — recursos Cloud SQL (`booksocial-db`, `db-f1-micro`, authorized `0.0.0.0/0`, `deletion_protection=false`) + BD + user (`booksocial`/`var.db_password`); Artifact Registry repo `apps`; **Cloud Run `identity` multi-container** (imagen `:latest`, port 8080, `SPRING_DATASOURCE_URL`, `SPRING_REDIS_HOST=localhost`, `SPRING_DATASOURCE_PASSWORD`, `APP_JWT_SECRET`, `FRONTEND_URL`, `SERVER_PORT=8080` + **sidecar `redis:7-alpine` con `command=["redis-server"]`**, min 0, 512Mi); **Cloud Run `gateway`** (`APP_JWT_SECRET`, `IDENTITY_SERVICE_URI=identity.uri`, `USER_SERVICE_URI`/`BOOK_SERVICE_URI` placeholders); IAM `allUsers` invoker (for_each identity+gateway).
...
- `infrastructure/terraform/environments/dev/variables.tf` — `project_id`, `region` (default `europe-west1`), `db_password`/`jwt_secret` (sensitive), `db_host`, `frontend_url`.
- `infrastructure/terraform/environments/dev/outputs.tf` — `project_id`, `gateway_url`, `identity_url`.
- `infrastructure/terraform/environments/dev/terraform.tfvars` — NO commiteada (secretos). `.terraform.lock.hcl` sí.
- Imágenes `:latest` de identity/gateway (y user/book) en `us-central1-docker.pkg.dev/booksocial-infra/apps/`.
